...
(defn add-contact [data owner]
  (let [contact-string (:text (om/get-state owner))
        new-contact (-> contact-string
                        parse-contact)]
    (when new-contact
  ...
  (reify
    ...
    (render-state [this {:keys [delete]}]
      (dom/li nil
              ...
              (dom/button #js {:onClick (fn [_] (put! delete @contact))} "Delete")))))
    ...
    (will-mount [_]
      (let [delete (om/get-state owner :delete)]
        (go (loop []
              ...
              (recur)))))
    om/IRenderState
    (render-state [this {:keys [delete text] :as state}]
      ...
      (dom/div nil
               ...
               (dom/div nil
                        ...
                        (dom/button
                         #js {:onClick #(add-contact data owner)}
                         "Add Contact"))))))
